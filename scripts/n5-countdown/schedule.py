...
def humanise(labels, spans, seen):
    """
    ...
    for lab in labels:
        ...
        title = THEME_TITLES.get(base, base)
...
SRC = REPO / 'lib' / 'constants' / 'n5-kanji.ts'
TEST_DATE = date(2026, 12, 6)
TEACHING_WEEKS = 12  # W13 = full review + mock, W14 = test week


def load_kanji():
    body = SRC.read_text().split('N5_KANJI: KanjiData[] = [', 1)[1]
    group, out = None, []
...
def build():
    ...
    for i, start in enumerate(starts):
        ...
        if i < TEACHING_WEEKS:
            new = weeks[i]
            theme = humanise(themes[i], spans, seen) or 'Consolidation'
        else:
            new = []
...
if __name__ == '__main__':
    ...
    out.write_text(json.dumps({'kanji': kanji, 'plan': plan}, ensure_ascii=False, indent=1))
    total_new = sum(len(w['new']) for w in plan)
    print(f'{len(kanji)} kanji -> {total_new} placed across {TEACHING_WEEKS} teaching weeks\n')
    for w in plan:
